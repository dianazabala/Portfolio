...
log_print("Subset dataset to CABG surgery patients")
...
log_print(claims_cabg)

#2) Identify comorbidities present in the diagnosis codes using either the Elixhauser (available 
#from AHRQ for SAS, or on SSC for Stata) or Charlson index #(either is fine for this exercise).

log_print("identify comorbidities using Charlson index")
...
comord_inds_unique <- unique(comord_inds) #make sure there are no duplicates

log_print(comord_inds_unique)

#output comorbidity information as csv file
write.csv(comord_inds_unique, "comorbidities.csv", row.names = FALSE)
...
#create a dataframe that is unique by patient ID, keeping the row 
#with the most recorded visits for each patient
claims_cabg_unique <- claims_cabg_dupes2 %>% filter(id==maxid)
length(unique(claims_cabg_unique$patientId)) #1721 rows

log_print("create dataframe that is unique by patient ID,  keeping the row with the most recorded visits for each patient with")
log_print("Include the number of days between hospital admission and an indicator for readmission")
log_print(claims_cabg_unique)
...
unique_claims_comorbid <- full_join(claims_cabg_unique, comord_inds_unique, by="patientId")
...
log_print("Create dataset for modeling with claims and comorbidity info and missing age values imputed")
log_print(unique_claims_comorbid)
...
print(table(unique_claims_comorbid$readmission))
92/(92+1629) ## over 5% readmissions compared to non-readmissions
...
m1 <- glm(readmission~ 
            ...
            ind_msld +
            ind_metacanc, 
            data = unique_claims_comorbid, family = binomial(link = "logit"))
...
library(broom) #library allows tidy output of regression  model
m2_output <- tidy(m2)
write.csv(m2_output, "model_output_tidy.csv", row.names = FALSE)

log_print("Create a logistic regression model to predict readmission likelihood, controlling for age, systolic blood pressure, and number of comorbidities")
log_print(m2_output)
...
extreme <- intersect(influential, standard_res) #NONE
...
####### Model 2 Concordance
library(survival)
print(concordance(m2)) # 78% concordant

####### Obtain Model 2 predictions (estimate likelihood of readmission for each patient as a probability)
unique_claims_comorbid$p_hat <- predict(m2, type='response')
print(summary(unique_claims_comorbid$p_hat))

####### Model 2 ROC and AUC
library(PredictABEL)
PredictABEL::plotROC(unique_claims_comorbid, "readmission", unique_claims_comorbid$p_hat)
#AUC [95% CI] for the model 1 :  0.785 [ 0.741  -  0.829 ] 


#output the dataset with predicted likelihood of readmission (phats)
write.csv(unique_claims_comorbid, "dataset_predictedvals.csv", row.names = FALSE)

log_print("create final dataset with estimates of readmission likelihood (predicted values")
log_print(unique_claims_comorbid)
